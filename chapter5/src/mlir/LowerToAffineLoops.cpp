...
namespace {
    ...
    struct BinaryOpLowering : public ConversionPattern {
        ...
        LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
            ...
            auto loc = op->getLoc();
            lowerOpToLoops(
                    op, operands, rewriter, [loc](OpBuilder &builder, ValueRange memRefOperands, ValueRange loopIvs) {
        ...
        LogicalResult matchAndRewrite(toy::ConstantOp op, PatternRewriter &rewriter) const final {
            ...
            std::function<void(uint64_t)> storeElements = [&](uint64_t dimension) {
                ...
                if (dimension == valueShape.size()) {
                    rewriter.create<affine::AffineStoreOp>(loc, rewriter.create<arith::ConstantOp>(loc, *valueIt++),
    ...
    struct ReturnOpLowering : public OpRewritePattern<toy::ReturnOp> {
        ...
        LogicalResult matchAndRewrite(toy::ReturnOp op, PatternRewriter &rewriter) const final {
    ...
    struct TransposeOpLowering : public ConversionPattern {
        TransposeOpLowering(MLIRContext *ctx) : ConversionPattern(toy::TransposeOp::getOperationName(), 1, ctx) {}
...
std::unique_ptr<Pass> mlir::toy::createLowerToAffinePass() { return std::make_unique<ToyToAffineLoweringPass>(); }
